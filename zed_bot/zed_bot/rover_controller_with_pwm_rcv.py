...
class RoverController(Node):
            ...
            if strt_idx != -1:
                ...
                try:
                    ...
                    self.get_logger().info(
                        f"[rcv_ard] Steer: {self.ard_steer_pwm}, Throttle: {self.ard_throttle_pwm}, Btn: {self.btn}")
                except struct.error as e:
                    self.get_logger().warn(f"Corrupted Arduino packet: {e}")
                    self.ard_received = False
            else:
                # self.get_logger().warn("[rcv_ard] No valid ARD header found in buffer")
                self.get_logger().info(f"[rcv_ard1] {buffer.decode(errors='ignore')}")
            self.ard_last_recv_time = time.time()
        elif len_buffer > 0:
    ...
    def xbee_vel_callback(self):
        steer_ang = self.steer_norm_scale * (self.ard_steer_pwm - self.steer_pwm_center)
        # self.get_logger().info(f"(steer_ang = {steer_ang:.2f})")
        self.v_ref = 0.0
        if self.ard_throttle_pwm >= self.v_pwm_min:
            self.v_ref = self.vel_fwd_norm_scale * (self.ard_throttle_pwm - self.v_pwm_min) + self.v_min
            self.v_ref = np.clip(self.v_ref, self.v_min, self.v_max)
            self.w_ref = self.v_ref * np.tan(steer_ang) / self.wheelbase
            self.w_ref = np.clip(self.w_ref, -self.w_max, self.w_max)
        elif self.ard_throttle_pwm <= self.v_pwm_bwd_min:
            self.v_ref = self.vel_bwd_norm_scale * (self.ard_throttle_pwm - self.v_pwm_bwd_max) + self.v_bwd_max
            self.v_ref = np.clip(self.v_ref, self.v_bwd_max, self.v_bwd_min)
            self.w_ref = self.v_ref * np.tan(steer_ang) / self.wheelbase
            # self.get_logger().info(f"(bwd_w1 = {self.w_ref:.2f})")
            self.w_ref = np.clip(self.w_ref, self.w_bwd_max, -self.w_bwd_max)
    ...
    #                     self.get_logger().info(f"rcv_struct: {rcv_struct.hex()}")
    #         else:
